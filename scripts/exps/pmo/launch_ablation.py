...
def _completed(matrix: dict[str, Any], job: Job) -> bool:
    summary_path = _run_dir(matrix, job) / "summary.json"
...
def _command(matrix_path: Path, matrix: dict[str, Any], job: Job) -> list[str]:
...
def main() -> None:
    ...
    pending = [job for job in _jobs(matrix) if not _completed(matrix, job)]
    print(
        json.dumps(
            {
    ...
    if args.dry_run:
        for job in pending:
            print("DRY RUN", job.label, _command(matrix_path, matrix, job), flush=True)
        return
    ...
    while pending or running:
        ...
        for gpu_index in free_worker_indices:
            ...
            if state.utilization_percent >= args.utilization_threshold:
                continue
            if state.memory_free_mib < args.min_free_memory_mib:
                continue

            job = pending.pop(0)
            command = _command(matrix_path, matrix, job)
            ...
            snapshot = {
                ...
                "compute_processes": [
                    row for row in _compute_process_snapshot() if row["gpu_uuid"] == state.uuid
                ],
                "command": command,
                "time_unix": time.time(),
            }
